...
const config = require('./configCommands.json');
const databasePath = path.join(__dirname, 'database.json');

const admins = ['admin1_fb_id', 'admin2_fb_id']; // Replace with actual Facebook IDs of the admins
...
async function handleCommand(message) {
  ...
  if (command.startsWith('autoreply ')) {
    if (admins.includes(message.senderID)) {
      const parts = command.split(' ', 2);
      const replyText = parts[1];

      const db = loadDatabase();
      db.autoReplies.push({ text: replyText });
      saveDatabase(db);

      message.reply('Auto-reply added!');
    } else {
      message.reply('You do not have permission to use this command.');
    }
  } else {
    const db = loadDatabase();
    const reply = db.autoReplies.find(r => message.body.includes(r.text));
...
// Example: client.on('message', handleCommand);
